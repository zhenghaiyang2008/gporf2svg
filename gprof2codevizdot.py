...
class SleepyParser(Parser):
    ...
    def openEntry(self, name):
        ...
        return self.database.open(name, 'r', newline='\n')
...
class Theme:
    ...
    def __init__(self, 
            ...
            minfontsize = 10.0,
            maxfontsize = 10.0,
            minlabeldistance = 0.5,
            maxlabeldistance = 4.0,
            minpenwidth = 0.5,
            maxpenwidth = 4.0,
            gamma = 2.2,
            skew = 1.0):
        ...
        self.minlabeldistance = minlabeldistance
        self.maxlabeldistance = maxlabeldistance
    ...
    def edge_labeldistance(self, weight):
            return max(weight*self.maxlabeldistance, self.minlabeldistance)
...
TEMPERATURE_COLORMAP = Theme(
    ...
    minfontsize = 15.0,
    maxfontsize = 15.0,
    minlabeldistance = 1.0,
    maxlabeldistance = 8.0,
)
...
def main():
    ...
    if Format.stdinInput:
        if not args:
            ...
            fp = open(args[0], 'rt', encoding='UTF-8', newline='\n')
        else:
            fp = open(args[0], 'rt', newline='\n')
        parser = Format(fp)
    ...
    if options.output is None:
        if PYTHON_3:
            output = open(sys.stdout.fileno(), mode='wt', encoding='UTF-8', closefd=False, newline='\n')
        else:
            output = sys.stdout
    else:
        if PYTHON_3:
            output = open(options.output, 'wt', encoding='UTF-8', newline='\n')
        else:
            output = open(options.output, 'wt', newline='\n')
